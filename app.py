...
def main():
    ...
    if a== 1:
        ...
        image = cv.resize(original_image, (640, 480))
        ...
        if len(hands) > 0:
            ...
            for rect in rects:
                # 回転考慮の領域の描画, 赤色の枠
                rects_tuple = ((rect[0]*original_width/640, rect[1]*original_height/480), (rect[2]*original_width/640, rect[3]*original_height/480), rect[4])
                box = cv.boxPoints(rects_tuple).astype(np.int0)
                # cv.drawContours(debug_image, [box], 0,(0,0,255), 2, cv.LINE_AA)

                # 回転非考慮の領域の描画, オレンジ色の枠
                rcx = int(rect[0]*original_width/640)
                rcy = int(rect[1]*original_height/480)
                half_w = int((rect[2]*original_width/640) // 2)
                half_h = int((rect[3]*original_height/480) // 2)
                x1 = rcx - half_w
                y1 = rcy - half_h
                x2 = rcx + half_w
                y2 = rcy + half_h
        ...
        if len(cropted_rotated_hands_images) > 0:
            ...
            if len(hand_landmarks) > 0:
                ...
                for (trackid, x1y1), landmark, rotated_image_size_leftright, not_rotate_rect in \
                    ...
                    lines = np.asarray(
                        [
                            np.array([[landmark[point][0]*original_width/640, landmark[point][1]*original_height/480] for point in line]).astype(np.int32) for line in lines_hand
                        ]
                    )                    
                    radius = int(1+thick_coef*5)+2
                    cv.polylines(
                        debug_image,
                        lines,
                        False,
                    ...
                    _ = [cv.circle(debug_image, (int(x*original_width/640), int(y*original_height/480)), radius, (0,128,255), -1) for x,y in landmark[:,:2]]
        ...
        # 画面反映 #############################################################
        cv.imwrite('Hand_Gesture_Recognition.png', debug_image)
